...
import de.troido.bleacon.scanner.NORDIC_ID
...
class BleFilterBuilder {
    internal val filter = ScanFilter.Builder()
    val manufacturerData = ManufacturerDataRelay(filter)
...
class ManufacturerDataRelay(private val filter: ScanFilter.Builder) {
    operator fun set(id: Int, mask: ByteArray = EMPTY, data: ByteArray) {
        filter.setManufacturerData(id, data, mask)
    }
}

fun bleFilter(build: BleFilterBuilder.() -> Unit): BleFilter =
        BleFilterBuilder().apply(build).let { BleFilter(it.filter.build(), it.dataTransform) }
